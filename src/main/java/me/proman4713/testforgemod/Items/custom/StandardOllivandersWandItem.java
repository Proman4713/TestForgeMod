package me.proman4713.testforgemod.Items.custom;
...
public class StandardOllivandersWandItem extends WandItem {
	...
	public void appendHoverText(ItemStack pStack, TooltipContext pContext, TooltipDisplay pTooltipDisplay, Consumer<Component> pTooltipAdder, TooltipFlag pFlag) {
		if (Screen.hasShiftDown()) {
			pTooltipAdder.accept(Component.translatable("tooltip.testforgemod.ollivanders_wand.expelliarmus"));
		} else {
			pTooltipAdder.accept(Component.translatable("tooltip.testforgemod.ollivanders_wand.lumos"));
		}
...
//			player.setIgnoreFallDamageFromCurrentImpulse(true);
		...
		if (level instanceof ServerLevel) {
			...
			MinecraftForge.EVENT_BUS.register(new LightRemovalHandler(level, posToChange, previousBlockState, 200));
		}
	}
	...
	void castOnAir(Level level, Player player, InteractionHand hand) {
		...
		for (int i = 0; i < 50; i++) {
			...
			serverLevel.sendParticles(
					new DustParticleOptions(0xFF0000, 3F),
		...
		for (LivingEntity target : targets) {
			...
			if (!held.isEmpty()) {
				target.drop(held.copy(), false, true);
				target.setItemInHand(InteractionHand.MAIN_HAND, ItemStack.EMPTY);

				serverLevel.sendParticles(
						new DustParticleOptions(0xFF0000, 1F),
				...
				level.playSound(null, target.blockPosition(), SoundEvents.ITEM_BREAK.value(), SoundSource.PLAYERS, 1.0f, 1.0f);
			}
		}
	}
	...
	void hitOnEntity(ItemStack stack, Player player, Entity entity) {
		...
		if (!held.isEmpty()) {
			...
			player.level().playSound(null, target.blockPosition(), SoundEvents.ITEM_BREAK.value(), SoundSource.PLAYERS, 1.0f, 1.0f);
		}
	}
